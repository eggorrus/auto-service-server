...
public class ClientHandler implements HttpHandler {
    ...
    private void handleGetAll(HttpExchange exchange) throws IOException, SQLException {
        ...
        try (Connection conn = DBConfig.getConnection();
            ...
            while (rs.next()) {
                clients.add(mapResultSetToJson(rs));
            }
        }

        sendResponse(exchange, 200, new JSONArray(clients).toString());
    }
    ...
    private void handlePost(HttpExchange exchange) throws IOException, SQLException {
        ...
        String sql = "INSERT INTO client (first_name, last_name, car, phone_number, info) " +
                "VALUES (?, ?, ?, ?, ?) RETURNING id";
        ...
        try (Connection conn = DBConfig.getConnection();
            ...
            ps.setString(3, json.getString("car"));
            ps.setString(4, json.getString("phoneNumber"));
            ps.setString(5, json.getString("info"));
    ...
    private void handlePut(HttpExchange exchange, long id) throws IOException, SQLException {
        ...
        String sql = "UPDATE client SET first_name = ?, last_name = ?, car = ?, " +
                "phone_number = ?, info = ? WHERE id = ?";
